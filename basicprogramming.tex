...
\setbeamerfont{note page}{family=\sffamily,size=\scriptsize} % You can change 'tiny' to the desired size
...
% title info
\title{Basic Programming with Python (workshop)}%\\ \includegraphics{karatedoklein.jpg}}
%\subtitle{Research Highlights}
\author{Dmitry Nikolaenko, Jiada Tu, Jordan Byers}
...
\note{
\begin{itemize}
    \item Welcome to our programming course. This course will teach you the basics of computer programming in Python language. We hope that you'll be able to transfer your knowledge to other languages easily, once you get the hang of ``\underline{thinking like a programmer}''. The basic principles are similar in most popular programming languages.

    \item Before we start, just some words about how this course will be organised. [mute microphones, switch on cameras, ask questions in chat or unmute yourselves, colleagues here to help, break at around 11am, presentation -- live coding (type along) -- practise etc.]
\end{itemize}
}
\end{frame}
...
\note{
\begin{itemize}
    \item There are many different ways of how to give a basic introduction into programming -- and I will give you some alternative material on the next slide -- but this is the order we will do it in: After giving a short intro into how to start, we will learn about \textit{data types}, \textit{variables} and \textit{basic instructions}.

	\item One major task that you probably want to use computer programs for is \textit{data processing}. Therefore, we'll look at how to get data in and out of your program next. In order to efficiently handle the data, we will then learn the fundamental control statements and how to best reuse and structure the code.
\end{itemize}
}
\end{frame}
...
\note{
    Here is a list of sources that I have used when I prepared this course. You can also find it on our ARC website under training resources. It is great for self-study, and you can use it after this course to continue on your own.
\begin{enumerate}
    \item \textit{Python Wiki}, as a website, is a good resource of Python knowledge. The link I give you here is to the site ``Python for Non-Programmers'', that has an extensive list of materials to learn Python if you have not programmed in any language before.
    \item \textit{How to think like a Computer Scientist}. Some of the structure and ideas of the first few chapters have been used for this course.
    \item \textit{A Whirlwind Tour of Python} has a faster pace, and seems to be targeted at a more technically-minded audience.
    \item \textit{Software Carpentry} is an organisation aimed at providing basic software training for everyone. They have a ``\underline{train the trainers}'' approach, which means that they provide material and training courses for software carpentry trainers. Thankfully, you are also allowed to access and use their material for free. This training course here gives a Python introduction at hands of a practical data processing task.
\end{enumerate}
%\newpage
}
\end{frame}
...
\note{
\begin{itemize}
    \item We'll talk about how to get a simple computer program running, the basics in terms of \textit{data types}, \textit{variables}, \textit{if-statements} and \textit{loops}, getting data in and out of the program, \textit{functions} and \textit{variable scope}, and a bit about \textit{comments}, \textit{error types} and their \textit{handling}.
    \item I hope that this course will give you a good foundation to continue your Python learning further. Programming is a lot about learning by doing. There are tons of online (and offline) resources as well as forums (online and offline) where you can ask for help.
\end{itemize}
}
...
\note<1>{
\begin{itemize}
    \item Why do we want to program? Well, first of all: Why do YOU want to program? I'd be interested to hear what your reason for attending this course is, and what you hope to do with your programming skills. Please, just unmute yourselves, or put your reasons in the chat and I will read them out.

    \item Do a poll on Sli.do (\url{https://admin.sli.do/event/nKCTATuocUuMWsTJ2KYYom/polls})

    \item Collect reasons why participants want to learn programming. Maybe we can summarize this as follows: We want the computer to do the work for us.
\end{itemize}
}
}
...
\note{
Now, how do we get the computer to do these things for us? This is what we are doing: We write the program in a language that we as human beings (with some training) can understand. As the computer won't be able to understand the code that way, we then compile it, that is translating it into a computer language. Then we execute the program and, given there're no mistakes, the computer will get working for us.
}
...
\note<2>{
 Please note that I'm oversimplifying here a bit with regards to the terminology. The point is that there're languages, such as Python, which we will be using in this course, that translate and execute the code directly, without us having to do a separate compilation step.
}
...
\note{
    Now let's get started! What do we do? As I said, we choose Python. There are two reasons for this choice:
\begin{enumerate}
    \item It is an interpreted language, which means we do not need to compile it each time that we change the code.
    \item It is currently very popular for a number of reasons, and people have asked us for a Python course.
\end{enumerate}
}
\end{frame}
...
\note{
\begin{itemize}
	\item We make things even easier for ourselves by using the \textbf{Jupyter Notebooks} infrastructure. However, we're not using real \textbf{Jupyter Notebooks}, we only use the infrastructure to type and execute our code. The reason I use this setup is that I want to take away the setup and installation hurdle from learning to program for you.
    \item I still want you to do the typing yourself, as that is a crucial bit in programming. I encourage you to have a play with what you've just learned throughout this lesson, get it into your fingers, think about every character you put into your program as it makes a difference. You'll find that out very soon for yourselves!
    \item My colleagues and myself are here to help you if you get stuck, or if you do not understand the reason for problems or errors that you might encounter.
    \item {\tiny The ``\textit{Whirlwind Tour to Python}'' book offers \textbf{Jupyter Notebooks}, if you are interested in trying them after this course.}
\end{itemize}
}
...
\begin{itemize}
    \item One is comments. When you are writing your code, your intentions and your thinking might seem obvious, and you just want to get it running. However, believe me: If someone else is looking at your code at some point, and this someone else might be you in two weeks time, things are very probably not that obvious anymore. I found myself puzzling over my own code and why on earth I have done things like that quite often. In order to put a comment in your code you have to tell the interpreter that this bit is just for the humans reading the code, not interesting for the computer. In Python this is done by putting a hash symbol in front of the comment. This can be either at the beginning or in the middle of a line.
\end{itemize}
}
...
\begin{frame}[fragile, wide]{Writing files (C-style)}
	...
	\end{lstlisting}
	\bigskip
	\footnotesize{see also \href{https://www.learnpython.org/en/String_Formatting}{https://www.learnpython.org/en/String\_Formatting}
	}
...
\begin{frame}[fragile, wide]{Writing files (f-strings)}
...
\begin{frame}[fragile, wide]{Reading files}
...
\begin{itemize}
	...
	\item \href{https://www.durham.ac.uk/research/institutes-and-centres/research-methods/research/research-methods-cafe/}{Research Methods Cafe}
